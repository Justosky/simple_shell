    ...
    else
    {
        ...
        else
        {
            while (Token != NULL)
            {
                ...
                Tokens[NumOfTok] = malloc(strlen(Token) + 1);  // Allocate memory for token
                ...
                strcpy(Tokens[NumOfTok], Token);  // Copy token contents
                //Tokens[NumOfTok][strlen(Token)] = '\0';  // Null-terminate the token
                NumOfTok++;
